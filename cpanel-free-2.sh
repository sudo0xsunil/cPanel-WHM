...
show_disclaimer() {
    ...
    echo ""
    echo -e "${CYAN}For legitimate testing, consider the official 15-day trial at: https://store.cpanel.net/trial${NC}"
    echo ""
...
check_system_requirements() {
    ...
    # Check disk space (minimum 20GB free) - Fixed to integer
    ROOT_SPACE=$(df -BG / | awk 'NR==2 {print $4}' | sed 's/G//' | cut -d. -f1)
...
install_cpanel() {
    ...
    # Run installer - Fixed: Use 'sh latest' instead of 'bash latest --force --skip_cloudlinux'
    sh latest 2>&1 | tee -a "$LOG_FILE"
...
bypass_license_check() {
    ...
    # Create modified License.pm that always returns valid - Added more common methods to prevent breaks
    cat > /usr/local/cpanel/Cpanel/License.pm << 'EOF'
package Cpanel::License;
use strict;
use warnings;
use base qw(Exporter);
our @EXPORT_OK = qw(is_valid check get_status is_trial get_expiration get_ip get_type);
...
EOF
    ...
    # Method 7: Create cron job to maintain bypass - Fixed logic and typo
...
if [[ ! -f /usr/local/cpanel/etc/cpanel.lisc ]]; then
    cat > /usr/local/cpanel/etc/cpanel.lisc << 'EOLIC'
-----BEGIN LICENSE-----
VALID=1
EXPIRY=9999999999
IP=0.0.0.0
...
# Ensure License.pm is patched - Fixed: Overwrite with patched version if not containing the bypass marker
if ! grep -q "sub is_valid { return 1; }" /usr/local/cpanel/Cpanel/License.pm 2>/dev/null; then
    cat > /usr/local/cpanel/Cpanel/License.pm << 'EOLPM'
package Cpanel::License;
use strict;
use warnings;
use base qw(Exporter);
...
display_bypass_info() {
    ...
    echo "• Some features may check license differently and not work"
    ...
    echo ""
    echo -e "${RED}Remember: This is for testing only. For full features without issues, use the official 15-day trial at https://store.cpanel.net/trial${NC}"
    echo ""
    echo "============================================================================"
}
main() {
    ...
    echo ""
    echo -e "${YELLOW}Recommendation: For reliable testing, uninstall this and use the official trial license.${NC}"
    echo ""
}
main "$@"
